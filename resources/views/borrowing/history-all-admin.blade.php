<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-full mx-auto sm:px-6 lg:px-8"> {{-- max-w-full agar tabel lebih lebar --}}
                            ...
                            <div>
                                ...
                                <select id="status_peminjaman" name="status_peminjaman" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    @foreach ($statuses as $status)
                                        <option value="{{ $status }}" {{ request('status_peminjaman') == $status ? 'selected' : '' }}>{{ $status }}</option>
                                    @endforeach
                                </select>
                            </div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            @if(in_array($borrowing->status, ['Dipinjam', 'Terlambat']))
                                                <button type="button" onclick="openReturnModal({{ $borrowing->id }}, '{{ $borrowing->inventaris->nama_alat }}')" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-200">
                                                    Tandai Kembali
                                                </button>
                                            @else
                                                -
                                            @endif
                                            {{-- Tambahkan link ke detail jika ada route 'admin.borrowing.show' --}}
    ...
    <div x-data="{ showModal: false, borrowingId: null, itemName: '' }" x-show="showModal" @keydown.escape.window="showModal = false" style="display:none;" class="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form :action="'{{ url('admin/permintaan-peminjaman') }}/' + borrowingId + '/return'" method="POST">
                    @csrf
                    @method('PATCH')
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500 dark:text-gray-400">
                                        Anda akan menandai barang "<span x-text="itemName" class="font-semibold"></span>" telah dikembalikan. Stok akan dikembalikan ke sistem.
                                    </p>
                                    <div class="mt-4">
                                        <x-label for="catatan_petugas_return" value="Catatan Pengembalian (Opsional)" />
                                        <textarea id="catatan_petugas_return" name="catatan_petugas" rows="3" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block mt-1 w-full"></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        function openReturnModal(borrowingId, itemName) {
            // Karena kita menggunakan Alpine.js di modal, kita bisa set variabelnya langsung
            // Ambil Alpine component data dari elemen yang memiliki x-data
